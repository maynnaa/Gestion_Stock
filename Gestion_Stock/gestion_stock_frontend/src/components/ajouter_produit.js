...
const ProductFormModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!serialNumberRegex.test(serialNumber)) {
      toast.error('Le numéro de série doit être composé de 2 lettres suivies de 4 chiffres.');
      return;
    }
  
    if (!selectedFournisseur) {
      toast.error('Le fournisseur doit être sélectionné.');
      return;
    }
    ...
    try {
      ...
      toast.success('Produit ajouté avec succès !'); // Success message
      ...
      toast.error('Une erreur est survenue lors de l\'ajout du produit.'); // Error message
    }
  };
